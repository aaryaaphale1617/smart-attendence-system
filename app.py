...
# SAFE unpacking with proper checks
result = authenticator.login(location="main")

if result is None:
    st.title("🔐 Smart Attendance System - Please Login")
    st.stop()
...
if auth_status:
    ...
    menu = st.sidebar.radio("Menu", ["Dashboard","Face Recognition","Manage Students","Manage Staff","Attendance","Reports","Settings"])

    if menu == "Dashboard":
        st.title("📊 Dashboard")
        st.write("System summary")
        s = pd.read_csv("data/students.csv")
        t = pd.read_csv("data/staff.csv")
        att = pd.read_csv("data/attendance.csv")
        col1, col2, col3 = st.columns(3)
        col1.metric("Students", len(s))
        col2.metric("Staff", len(t))
        col3.metric("Attendance Records", len(att))

        st.subheader("Attendance Graph (by Name)")
        if len(att) > 0:
            summary = att.groupby("Name").size().sort_values(ascending=False).head(10)
            fig, ax = plt.subplots()
    ...
    if menu == "Face Recognition":
        ...
        st.write("When you press Start Attendance, the camera runs and marks both student and staff attendance automatically.")
        duration = st.slider("Recognition duration (seconds)", min_value=5, max_value=60, value=15)
        if st.button("Start Attendance Now"):
            frame, detected = start_recognition_session(mark_all=True, duration_sec=duration)
            st.success(f"Recognition finished. Detected: {', '.join(detected) if detected else 'None'}")
